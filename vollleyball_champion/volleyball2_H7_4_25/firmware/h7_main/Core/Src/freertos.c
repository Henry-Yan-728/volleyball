...
void StartDefaultTask(void *argument)
{
  ...
  uint32_t last_chassis_loop_tick = 0U;
  uint32_t last_chassis_safety_tick = 0U;
  ...
  for(;;)
  {
    ...
    if ((HAL_GetTick() - last_chassis_loop_tick) >= 5U)
    {
      Chassis_Task_Loop();
      last_chassis_loop_tick = HAL_GetTick();
    }
    if ((HAL_GetTick() - last_chassis_safety_tick) >= 50U)
    {
      Chassis_SafetyTick();
      last_chassis_safety_tick = HAL_GetTick();
    }
...
void StartTask02(void *argument)
{
  ...
  for (;;)
  {
      ...
      if (local_rc.mode == CHASSIS_MODE_MANUAL)
      {
          ...
          Robot_Pose_t pose_snapshot;

          target_vx = local_rc.vx*5;
          target_vy = local_rc.vy*5;
          target_vr = local_rc.vw;
          auto_planner_active = 0U;
          serve_planner_active = 0U;

          Robot_Data_GetPoseSnapshot(&pose_snapshot);
          cur_yaw = pose_snapshot.angle;
          pose_update_tick = pose_snapshot.update_tick;
      ...
      else if (local_rc.mode == CHASSIS_MODE_AUTO)
      {
          ...
          Robot_Pose_t pose_snapshot;
          Robot_Target_t target_snapshot;

          Robot_Data_GetPoseSnapshot(&pose_snapshot);
          Robot_Data_GetTargetSnapshot(&target_snapshot);
          cur_x = pose_snapshot.x;
          cur_y = pose_snapshot.y;
          cur_yaw = pose_snapshot.angle;
          cur_vx = pose_snapshot.vx;
          cur_vy = pose_snapshot.vy;
          last_update = pose_snapshot.update_tick;
          auto_target_x = target_snapshot.target_x;
          auto_target_y = target_snapshot.target_y;
          cur_yaw *= DEG_TO_RAD;
...
void StartTask05(void *argument)
{
  ...
  #define CUSHION_READY_DEG   60.0f
...
void StartHmiTask(void *argument)
{
  ...
  (void)argument;
  HmiPanel_Init(&huart4);

  for (;;)
  {
      HmiPanel_Process();
      HmiPanel_UpdateScreen();
      osDelay(200U);
  }
...
static void freertos_require_handle(const void *handle, const char *name)
{
    ...
    printf("[RTOS] handle create failed: %s\r\n", (name != NULL) ? name : "unknown");
    Error_Handler();
}

static void planner_tuning_send_value(uint8_t cmd_type, uint8_t param_id, float value)
{
    uint8_t frame[PLANNER_TUNING_FRAME_LENGTH];

    if (PlannerTuning_BuildFrame(cmd_type, param_id, value, frame) == 0U) {
        return;
    }

    (void)HAL_UART_Transmit(&huart10, frame, sizeof(frame), 20U);
}
...
static uint8_t planner_tuning_try_read_value(uint8_t param_id, float *value_out)
{
    ...
    switch (param_id) {
        case PID_MAX_SPD: *value_out = cfg.max_spd; return 1U;
        case PID_START_SPD: *value_out = cfg.start_spd; return 1U;
        case PID_STOP_SPD: *value_out = cfg.stop_spd; return 1U;
        case PID_UP_DIST: *value_out = cfg.up_dist; return 1U;
        case PID_DOWN_DIST: *value_out = cfg.down_dist; return 1U;
        case PID_ANGLE_KP: *value_out = cfg.angle_kp; return 1U;
        case PID_MAX_VR: *value_out = cfg.max_vr; return 1U;
        case PID_VR_SLEW_STEP: *value_out = cfg.vr_slew_step; return 1U;
        case PID_YAW_DEADZONE: *value_out = cfg.yaw_deadzone; return 1U;
        case PID_FAR_NEAR_DIST: *value_out = cfg.far_near_dist; return 1U;
        case PID_FAR_WEIGHT_MIN: *value_out = cfg.far_weight_min; return 1U;
        case PID_FAR_MAX_VR_SCALE: *value_out = cfg.far_max_vr_scale; return 1U;
        case PID_FAR_VR_SLEW_SCALE: *value_out = cfg.far_vr_slew_scale; return 1U;
        case PID_POS_TOLERANCE: *value_out = cfg.pos_tolerance; return 1U;
        case PID_YAW_TOLERANCE: *value_out = cfg.yaw_tolerance; return 1U;
        case SYS_SAVE_TO_FLASH:
        case SYS_RESET_DEFAULT:
            *value_out = 1.0f;
            return 1U;
        default:
            return 0U;
...
static uint8_t planner_tuning_try_write_value(uint8_t param_id, float value)
{
    ...
    if (param_id == SYS_RESET_DEFAULT) {
        Planner_Init(s_default_planner_cfg);
        return 1U;
    }

    if (param_id == SYS_SAVE_TO_FLASH) {
        return 1U;
    }
...
static void planner_tuning_process_frame(const PlannerTuningFrame_t *frame)
{
    ...
    if (frame->cmd_type == PLANNER_TUNING_CMD_READ) {
        ok = planner_tuning_try_read_value(frame->param_id, &value);
        if (ok != 0U) {
            planner_tuning_send_value(PLANNER_TUNING_CMD_READ, frame->param_id, value);
        }
    } else if (frame->cmd_type == PLANNER_TUNING_CMD_WRITE) {
        ok = planner_tuning_try_write_value(frame->param_id, frame->value);
        if (ok != 0U) {
            if (planner_tuning_try_read_value(frame->param_id, &value) == 0U) {
                value = frame->value;
            }
            planner_tuning_send_value(PLANNER_TUNING_CMD_WRITE, frame->param_id, value);
        }
    }

    if (ok == 0U) {
        printf("[PlannerTuning] unsupported cmd=0x%02X param=0x%02X value=%.3f\r\n",
               frame->cmd_type,
               frame->param_id,
               (double)frame->value);
    }
...
static float manual_attitude_update_vr(float now_yaw_rad)
{
    ...
    yaw_err = manual_attitude_apply_soft_deadzone(yaw_err, cfg.yaw_deadzone);

    target_vr = manual_attitude_clampf(yaw_err * cfg.angle_kp, -cfg.max_vr, cfg.max_vr);
    delta_vr = manual_attitude_clampf(target_vr - s_manual_attitude_last_vr,
                                      -cfg.vr_slew_step,
                                      cfg.vr_slew_step);

    s_manual_attitude_last_vr += delta_vr;
    s_manual_attitude_last_vr = manual_attitude_clampf(s_manual_attitude_last_vr,
                                                       -cfg.max_vr,
                                                       cfg.max_vr);
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART10) {
        ...
        HAL_UARTEx_ReceiveToIdle_DMA(huart, remote_Buffer, RC_BUFFER_SIZE);
        __HAL_DMA_DISABLE_IT(huart->hdmarx, DMA_IT_HT);
    } else if (HmiPanel_IsUart(huart) != 0U) {
        __HAL_UART_CLEAR_FLAG(huart, UART_FLAG_PE | UART_FLAG_FE | UART_FLAG_ORE | UART_FLAG_NE);
        HmiPanel_RestartRx();
    }
}
