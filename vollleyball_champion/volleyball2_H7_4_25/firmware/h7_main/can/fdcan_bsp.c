...
static void fdcan_drain_rx_fifo(FDCAN_HandleTypeDef* hfdcan, uint32_t fifo_location);
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
    {
        ...
        fdcan_drain_rx_fifo(hfdcan, FDCAN_RX_FIFO0);
    }
}
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo1ITs)
{
    ...
    {
        fdcan_drain_rx_fifo(hfdcan, FDCAN_RX_FIFO1);
    }
}
...
static void fdcan_drain_rx_fifo(FDCAN_HandleTypeDef* hfdcan, uint32_t fifo_location)
{
    ...
    {
        if (fdcan_dispatch_router(hfdcan, fifo_location) == 0U)
        {
            break;
        }
    }
