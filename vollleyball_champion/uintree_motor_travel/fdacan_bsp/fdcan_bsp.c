...
static void fdcan_dispatch_router(FDCAN_HandleTypeDef* hfdcan, uint32_t fifo_location)
{
    ...
    // 如果是 CAN3 且是 扩展帧 (Extended ID)，则判定为宇树电机
    if (hfdcan->Instance == FDCAN3 && rx_header.IdType == FDCAN_EXTENDED_ID)
    {
        Unitree_Rx_Handler(&rx_header, rx_data);
        return; // 处理完毕，直接返回
    }
    
    // --- 拦截逻辑结束 ---

    // 下面是原有的标准帧处理逻辑 (DJI电机、定位板等)
