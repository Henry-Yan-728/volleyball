...
#define SMALL_BOX_HEIGHT 150        // 小纸箱高度 (mm)
#define BIG_BOX_HEIGHT 180          //大纸箱高度（mm）
...
static void move_to_position(float radius, float angle, float height,float xiaobi_angle)
{
	    if (osMutexAcquire(motorMutexHandle, osWaitForever) == osOK) 
    {
    ...
    dabi_control(radius);
        ...
        // 释放电机资源
        osMutexRelease(motorMutexHandle);
    }
