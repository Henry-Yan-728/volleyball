...
// --- 规划器参数 ---
// 底盘最大平滑速度 (rad/s)         - 【调参】
#define PLANNER_MAX_VELOCITY        (15.00f)
// 底盘最大平滑加速度 (rad/s^2)     - 【调参】
#define PLANNER_MAX_ACCELERATION    (4.0f)
...
static void prv_planner_task(void *pvParameters)
{
    ...
    for(;;)
    {
        ...
        // B. 线程安全地获取最终目标
        float local_target;
        xSemaphoreTake(g_can_tx_mutex, portMAX_DELAY);
...
int8_t trajectory_planner_start(CyberGear_Motor_Instance* motor)
{
    ...
    if (xPlannerTaskHandle == NULL || g_can_tx_mutex == NULL)
    {
