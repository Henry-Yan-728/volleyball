...
#define time_add 1
	...
	float time_ms = 1;
  float PITCH = 4.06f;                 // Pitch 轴力度控制变量
...
void StartTask02(void *argument)
{
  ...
  // --- 2. 机构动作参数配置 (角度/速度) ---
  const float CUSHION_READY_DEG = 47.0f;   // 垫球机构复位角度 (deg)
  ...
  for(;;)
  {
      ...
      if (local_rc.mode == CHASSIS_MODE_MANUAL) 
      {
          ...
          target_vr = -local_rc.vw * 3.0f;   // 自旋

// 定义消抖时间阈值 (通常 20ms - 50ms)
#define DEBOUNCE_DELAY_MS 10

// 定义静态变量保存上一次触发的时间 (也可以放在全局结构体中)
...
if (local_rc.button5 == 1 && last_button5 == 0) {
    ...
    if ((current_tick - last_time_btn5) > DEBOUNCE_DELAY_MS) {
        ...
        last_time_btn5 = current_tick; // 更新触发时间
    }
}
last_button5 = local_rc.button5;

// --- Button 2 & 4 处理 (Pitch 角度调节) ---
// Button 2
...
last_button2 = local_rc.button2; // 独立保存状态
...
last_button4 = local_rc.button4; // 独立保存状态

// --- Button 1 & 3 处理 (PITCH 微调) ---
// Button 1
...
last_button1 = local_rc.button1; // 独立保存状态
...
last_button3 = local_rc.button3; // 独立保存状态
...
void StartTask07(void *argument)
{
  ...
  {
    FDCAN_TxHeaderTypeDef TxHeader;
  uint8_t TxData[12]; // 4字节时间戳 + 4字节Yaw + 4字节Pitch
  
  // 初始化经典 CAN 发送帧头
  TxHeader.Identifier = CAN_ID_PC_FEEDBACK; // 0x300
  TxHeader.IdType = FDCAN_STANDARD_ID;
  TxHeader.TxFrameType = FDCAN_DATA_FRAME;
  TxHeader.DataLength = FDCAN_DLC_BYTES_8;  // 退回传统的 8 Bytes
  TxHeader.ErrorStateIndicator = FDCAN_ESI_ACTIVE;
  TxHeader.BitRateSwitch = FDCAN_BRS_OFF;   // 关闭速率切换 (经典CAN必须OFF)
  TxHeader.FDFormat = FDCAN_CLASSIC_CAN;    // 使用传统经典 CAN 格式
  TxHeader.TxEventFifoControl = FDCAN_NO_TX_EVENTS;
  TxHeader.MessageMarker = 0;
  
  TickType_t xLastWakeTime = osKernelGetTickCount();
  const TickType_t xFrequency = pdMS_TO_TICKS(1); // 发送频率 1000Hz (1ms)
  ...
  for(;;)
  {
      ...
      // 数据压缩转换 (Float角度 * 100 转换为 int16_t，保留两位小数精度)
      int16_t yaw_send   = (int16_t)(yaw_angle * 100.0f);
      int16_t pitch_send = (int16_t)(pitch_angle * 100.0f);

      // 数据打包
      memcpy(&TxData[0], &current_timestamp, 4); // [0-3]: uint32_t 时间戳
      memcpy(&TxData[4], &yaw_send, 2);          // [4-5]: int16_t Yaw 角度(*100)
      memcpy(&TxData[6], &pitch_send, 2);        // [6-7]: int16_t Pitch 角度(*100)
