...
{
    /* USART2 已配置为硬件 RS485（DE 自动控制），无需手动切换方向脚。 */
    (void)tx_enable;
}
...
{
    printf("[Unitree] 电机控制初始化 - USART2 RS485\r\n");
    UnitreeMotor_SetDirection(0); // 硬件 DE 自动控制，保留旧接口调用
}
...
void UnitreeMotor_SendCmd(MotorCmd_t *cmd)
{
    ...
    UnitreeMotor_SetDirection(1); // 硬件 DE 自动控制，保留旧接口调用
