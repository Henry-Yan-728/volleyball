...
void StartTask02(void *argument)
{
  ...
  float PITCH = 5400.0f;                 // Pitch 轴力度控制变量
  ...
  for(;;)
  {
      ...
      switch (serve_state) {
          case ACTION_STEP_1:
              // 动作：垫球机构配合抬起，发球电机转动
								Mechanism_Cushion_SetAngle(CUSHION_READY_DEG,CUSHION_ACTION_DEG, PITCH);
          ...
          case ACTION_STEP_2:
              // 动作：垫球机构复位，发球电机保持转动确保球射出
								Mechanism_Cushion_SetAngle(CUSHION_ACTION_DEG,CUSHION_READY_DEG, 1080.0f);
								Mechanism_Serve_SetAngle(SERVE_ACTION_RAD);
...
void StartTask05(void *argument)
{
  /* USER CODE BEGIN StartTask05 */
    ...
    #define CUSHION_SPEED       5400.0f
    #define ACTION_HOLD_MS      800
    const float CUSHION_ACTION_DEG = 114.0f;

    osDelay(1000); // 上电延时，等待传感器稳定
    // 初始化击球板位置
		Mechanism_Cushion_SetAngle(CUSHION_READY_DEG,CUSHION_READY_DEG, CUSHION_SPEED);
  ...
  for(;;)
  {
    ...
    if (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_11) == GPIO_PIN_RESET)
    {
        ...
        if (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_11) == GPIO_PIN_RESET)
        {
            ...
            // 击球动作开始
            Mechanism_Cushion_SetAngle(CUSHION_READY_DEG,CUSHION_ACTION_DEG, CUSHION_SPEED);
