...
float current_yaw_target = 0.0f;
float current_pitch_target = 0.0f;

UnitreePlanner_t joint1;
UnitreePlanner_t joint2;
UnitreePlanner_t joint3;
...
void Mechanism_Init(void)
{
    ...
    gimbal_control_init();
    HAL_Delay(100);
    gimbal_set_angle(current_yaw_target,current_pitch_target);
	    Unitree_Init(&joint1, 1);
	    Unitree_Init(&joint2, 2);
	    Unitree_Init(&joint3, 3);
...
// =============================================================
void Mechanism_Cushion_SetAngle(float start_angle,float end_angle, float speed)
{
	  Unitree_SetTrajectory(&joint1, start_angle, end_angle, speed, 120.0f, 1.0f);
    Unitree_SetTrajectory(&joint2, start_angle, end_angle, speed, 120.0f, 1.0f);
    Unitree_SetTrajectory(&joint3, start_angle, end_angle, speed, 120.0f, 1.0f);
...
void Update_Virtual_Axis(void)
{
    ...
    {
        // 规范化转化: 角度 -> 编码器值
        // 使用宏 EXTERNAL_MECHANISM_RATIO (19.203 * 3) 
...
void unitree_update(void)
{
    Unitree_UpdateAndSend(&joint1, 0.001f);
    Unitree_UpdateAndSend(&joint2, 0.001f);
    Unitree_UpdateAndSend(&joint3, 0.001f);
}
