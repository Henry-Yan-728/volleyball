...
static void cybergear_motor_configure(uint8_t index, const uint8_t id, FDCAN_HandleTypeDef* hfdcan)
{
    ...
    // 参考“平滑”工程：同时固定源电机ID(bit15~8)和主机ID(bit7~0)
    // 这样能同时覆盖 type2 反馈和 type17 参数读取回复。
    dispatch_item.id = ((uint32_t)id << 8) | (uint32_t)MASTER_CAN_ID;
    dispatch_item.mask = (uint32_t)(0x0000FF00) | (uint32_t)(0x000000FF);
...
static uint8_t cybergear_can_send_raw(CyberGear_Motor_Instance* motor, 
                                      ...
                                      uint8_t tx_data[8])
{
    if (motor == NULL || motor->hfdcan == NULL || tx_data == NULL) return 0U;
    ...
    // 3. 发送报文（短重试，避免瞬时 FIFO 满导致整机进入 Error_Handler）
    uint8_t send_ok = 0;
