...
#define CAN_ID_PC_FEEDBACK       0x300  // 发送给PC的时间戳与云台角度反馈 (12 Bytes)
 ...
 */
typedef struct {
    float target_x;     // 目标 X 坐标 (mm)
    float target_y;     // 目标 Y 坐标 (mm)
    uint32_t update_tick; // 最后一次更新的时间戳
    uint8_t is_valid;   // 当前目标是否有效
