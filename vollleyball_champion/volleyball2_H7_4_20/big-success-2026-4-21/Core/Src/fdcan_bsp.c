...
#define FDCAN_RX_DRAIN_LIMIT 16U
...
static void fdcan_dispatch_drain(FDCAN_HandleTypeDef* hfdcan, uint32_t fifo_location);
...
void fdcan_bsp_start(FDCAN_HandleTypeDef* hfdcan)
{
    ...
    if (hfdcan->Instance == FDCAN3)
    {
        ...
        sFilterConfig.FilterID1 = 0x0AAU;
        sFilterConfig.FilterID2 = 0x0ABU;
        ...
        sFilterConfig.FilterID1 = 0x0ACU;
        sFilterConfig.FilterID2 = 0x100U;
        if (HAL_FDCAN_ConfigFilter(hfdcan, &sFilterConfig) != HAL_OK)
        {
            Error_Handler();
        }
        ...
        sFilterConfig.FilterID2 = 0x200U;
        if (HAL_FDCAN_ConfigFilter(hfdcan, &sFilterConfig) != HAL_OK)
        {
            Error_Handler();
        }
        ...
        sFilterConfig.FilterID2 = 0x00000C00U;
        if (HAL_FDCAN_ConfigFilter(hfdcan, &sFilterConfig) != HAL_OK)
        {
            Error_Handler();
        }
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    if((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
    {
        ...
        fdcan_dispatch_drain(hfdcan, FDCAN_RX_FIFO0);
    }
}
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo1ITs)
{
    ...
    {
        fdcan_dispatch_drain(hfdcan, FDCAN_RX_FIFO1);
    }
}
...
static void fdcan_dispatch_drain(FDCAN_HandleTypeDef* hfdcan, uint32_t fifo_location)
{
    ...
    while ((HAL_FDCAN_GetRxFifoFillLevel(hfdcan, fifo_location) > 0U) &&
           (drained < FDCAN_RX_DRAIN_LIMIT))
    {
        fdcan_dispatch_router(hfdcan, fifo_location);
        drained++;
    }
}
