...
void Mechanism_Cushion_SetAngle(float angle_deg, float kp)
{
    ...
    (void)Unitree_Send_Broadcast_Cmd(0.0f, 0.0f, target_rad, kp, kd_default);
}
...
void Update_Virtual_Axis(void)
{
    ...
    if ((m_right != NULL) && (m_left != NULL))
    {
        ...
        dji_motor_set_location(m_left, encoder_pos+68);
        dji_motor_set_location(m_right, -encoder_pos-45);
    }
}
