...
#define GIMBAL_YAW_HOME_TARGET_DEG     (83.0f)
#define GIMBAL_YAW_HOME_TOLERANCE_DEG  (1.0f)
#define GIMBAL_YAW_HOME_TIMEOUT_MS     3000U
#define GIMBAL_YAW_OFFLINE_TIMEOUT_MS  100U
#ifndef M_PI
...
static uint8_t s_gimbal_yaw_has_homed = 0U;

volatile uint32_t g_gimbal_yaw_home_state = GIMBAL_YAW_HOME_WAIT_ONLINE;
volatile int32_t g_gimbal_yaw_home_error = 0;

static uint8_t prv_gimbal_yaw_is_online(const DJI_Motor_Instance* yaw_motor)
{
    if ((yaw_motor == NULL) || (yaw_motor->is_online == 0U))
    {
        return 0U;
    }

    return (uint8_t)((HAL_GetTick() - yaw_motor->last_msg_time) <= GIMBAL_YAW_OFFLINE_TIMEOUT_MS);
}
...
static uint8_t prv_gimbal_yaw_home_update(DJI_Motor_Instance* yaw_motor)
{
    ...
    if (prv_gimbal_yaw_is_online(yaw_motor) == 0U)
    {
        if (s_gimbal_yaw_has_homed == 0U)
        {
