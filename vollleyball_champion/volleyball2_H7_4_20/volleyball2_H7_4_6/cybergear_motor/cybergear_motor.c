/**
  ...
  * 
  * [FreeRTOS 线程安全支持]
  * 1. 在 FreeRTOS 等多任务环境下，不同的任务可能同时调用本驱动的 API 函数来发送 CAN 指令。
  * 2. 发送路径统一委托给 `fdcan_bsp_send()`，由底层 BSP 负责按总线加锁和仲裁，
  *    避免业务模块各自维护私有锁导致的并发冲突。
  *
  ******************************************************************************
  */
...
typedef enum {
    ...
    CYBERGEAR_COMM_MOTION_CTRL   = 1,
    CYBERGEAR_COMM_FEEDBACK      = 2,
    CYBERGEAR_COMM_ENABLE        = 3,
    CYBERGEAR_COMM_STOP          = 4,
    CYBERGEAR_COMM_SET_ZERO      = 6,
    CYBERGEAR_COMM_READ_PARAM    = 17,
    CYBERGEAR_COMM_WRITE_PARAM   = 18,
    CYBERGEAR_COMM_FAULT         = 21,
} CyberGear_Comm_Type_e;
...
volatile uint32_t g_cybergear_rx_count = 0U;
volatile uint32_t g_cybergear_last_rx_id = 0U;
volatile uint8_t g_cybergear_last_comm_type = 0U;
volatile float g_cybergear_last_angle = 0.0f;
...
// 发送CAN报文的底层辅助函数
static uint8_t cybergear_can_send_raw(CyberGear_Motor_Instance* motor, uint8_t comm_type, uint16_t data2, uint8_t target_id, uint8_t tx_data[8]);

// 使能电机
static uint8_t motor_enable(CyberGear_Motor_Instance* motor);

// 向电机写入一个浮点型(float)参数
static uint8_t cybergear_write_float_parameter(CyberGear_Motor_Instance* motor, uint16_t index, float value);

// 向电机写入一个单字节(uint8_t)参数
static uint8_t cybergear_write_u8_parameter(CyberGear_Motor_Instance* motor, uint16_t index, uint8_t value);
...
void cybergear_motors_register_dispatches(void)
{
    for (uint8_t i = 0; i < CYBERGEAR_MOTOR_COUNT; i++)
    {
        CyberGear_Motor_Instance* motor = &cybergear_motors[i];
        ...
        dispatch_item.id = ((uint32_t)motor->can_id << 8) | (uint32_t)MASTER_CAN_ID;
        dispatch_item.mask = (uint32_t)(0x0000FF00U) | (uint32_t)(0x000000FFU);
        dispatch_item.instance_ptr = motor;
        dispatch_item.handler = cybergear_message_handler;
...
void cybergear_motor_set_mode(CyberGear_Motor_Instance* motor, CyberGear_Control_Mode_e mode)
{
    ...
    // 2. 根据目标模式，发送参数写入指令
    (void)cybergear_write_u8_parameter(motor, CYBERGEAR_PARAM_RUN_MODE, (uint8_t)mode);
...
void cybergear_motor_set_position(CyberGear_Motor_Instance* motor, float position, float speed_limit)
{
    ...
    // 2. 设置位置模式下的速度限制
    speed_limit = clampf(speed_limit, 0.0f, V_MAX);
    (void)cybergear_write_float_parameter(motor, CYBERGEAR_PARAM_LIMIT_SPD, speed_limit);
    
    // 3. 设置目标位置
    (void)cybergear_write_float_parameter(motor, CYBERGEAR_PARAM_LOC_REF, position);
}
...
void cybergear_motor_set_speed(CyberGear_Motor_Instance* motor, float speed)
{
    ...
    // 2. 设置目标速度
    speed = clampf(speed, V_MIN, V_MAX);
    (void)cybergear_write_float_parameter(motor, CYBERGEAR_PARAM_SPD_REF, speed);
}
...
void cybergear_motor_set_torque(CyberGear_Motor_Instance* motor, float torque)
{
    ...
    iq_ref = clampf(iq_ref, -23.0f, 23.0f);
    ...
    // 2. 设置目标Iq电流
    (void)cybergear_write_float_parameter(motor, CYBERGEAR_PARAM_IQ_REF, iq_ref);
}
...
void cybergear_motor_set_pid_gains(CyberGear_Motor_Instance* motor, float loc_kp, float spd_kp, float spd_ki)
{
    ...
    // 写入位置环 Kp (地址 0x2016)
    (void)cybergear_write_float_parameter(motor, 0x2016, loc_kp);

    // 写入速度环 Kp (地址 0x2014)
    (void)cybergear_write_float_parameter(motor, 0x2014, spd_kp);

    // 写入速度环 Ki (地址 0x2015)
    (void)cybergear_write_float_parameter(motor, 0x2015, spd_ki);
}
...
void cybergear_motor_set_current_limit(CyberGear_Motor_Instance* motor, float current_limit)
{
    ...
    // 设置一个安全钳位
    current_limit = clampf(current_limit, 0.0f, 23.0f);

    (void)cybergear_write_float_parameter(motor, CYBERGEAR_PARAM_LIMIT_CUR, current_limit);
}
...
{
    if (instance == NULL || rx_header == NULL || rx_data == NULL) return;
    
    // 将接收到的实例转换回电机实例
    CyberGear_Motor_Instance* motor = (CyberGear_Motor_Instance*)instance;
    motor->is_online = 1;
    motor->last_msg_time = HAL_GetTick();
    g_cybergear_rx_count++;
    g_cybergear_last_rx_id = rx_header->Identifier;

    // 从扩展ID中解析出通信类型
    uint8_t comm_type = (rx_header->Identifier >> 24) & 0x1F;
    g_cybergear_last_comm_type = comm_type;

    // 根据通信类型，分发到不同的处理逻辑
    switch(comm_type) {
        case CYBERGEAR_COMM_FEEDBACK: // 类型2: 电机通用反馈报文
        {
            // 解析模式位和故障位
            ...
            g_cybergear_last_angle = motor->measure.angle;
            break;
        }
        case CYBERGEAR_COMM_READ_PARAM: // 类型17: 单个参数读取的回复报文
        {
            // 解析回复的参数地址和值
            ...
            switch(param_index) {
                ...
                case PARAM_IQ_FILTER: // iq滤波值(A), float
                {
                    float iq;
                    memcpy(&iq, &rx_data[4], sizeof(float));
                    // 近似换算为转矩，便于上层统一读取
                    motor->measure.torque = iq * 0.87f;
                    break;
                }
        ...
        // 对于其他通信类型，暂时不关心
        // 后续有需要时可添加相关处理逻辑
        case CYBERGEAR_COMM_FAULT: // 类型21: 故障反馈报文
        {
            uint32_t fault = 0;
            memcpy(&fault, &rx_data[0], sizeof(uint32_t));
            motor->measure.fault = (fault != 0U) ? 1U : 0U;
            break;
        }
        default: break;
    }
 ...
 */
static uint8_t cybergear_can_send_raw(CyberGear_Motor_Instance* motor, 
                                      uint8_t comm_type, 
                                      uint16_t data2, 
                                      uint8_t target_id, 
                                      uint8_t tx_data[8])
{
    ...
    tx_header.Identifier = (((uint32_t)comm_type & 0x1FU) << 24) | 
                           (((uint32_t)data2 & 0xFFFFU) << 8)    | 
                           ((uint32_t)target_id);
    ...
    // 3. 通过统一 BSP 发送接口入总线，复用按总线维度的互斥与错误统计
    uint8_t send_ok = 0;
    for (uint8_t retry = 0; retry < 3; retry++)
    {
        if (fdcan_bsp_send(motor->hfdcan, &tx_header, tx_data) == 0U)
        {
            send_ok = 1;
            break;
        }
