...
#define MECH_CUSHION_READY_DEG   57.0f
...
#define PAN_TILT_CMD_TIMEOUT_MS  100U
...
static void mechanism_restore_busy_state_if_needed(void)
{
    PC_state current_state = Get_System_Doit_State();

    if (current_state == START_BUSY) {
        Set_System_Doit_State(START);
    } else if (current_state == OVER_BUSY) {
...
void Mechanism_Loop_1ms(void)
{
    ...
    uint32_t pan_tilt_cmd_update_tick;
    uint32_t now_tick = HAL_GetTick();

    taskENTER_CRITICAL();
    yaw_speed_cmd = current_yaw_speed;
    pitch_target_deg = current_pitch_target_deg;
    pan_tilt_cmd_update_tick = g_pan_tilt_cmd_update_tick;
    taskEXIT_CRITICAL();

    if ((pan_tilt_cmd_update_tick == 0U) ||
        ((now_tick - pan_tilt_cmd_update_tick) > PAN_TILT_CMD_TIMEOUT_MS)) {
        gimbal_get_angles(NULL, &current_pitch_deg);
        yaw_speed_cmd = 0.0f;
        pitch_target_deg = current_pitch_deg;
    }

    gimbal_set_speed(yaw_speed_cmd, pitch_target_deg);
...
void Mechanism_ServeController_Cancel(MechanismServeController_t *controller)
{
    ...
    mechanism_serve_reset_controller(controller);
    mechanism_serve_hard_stop_outputs();
    mechanism_restore_busy_state_if_needed();
}
                                       ...
                                       float cushion_kp)
{
    ...
    switch ((ServeControllerState_e)controller->action_state) {
        ...
        case SERVE_CTRL_STEP2:
            Chassis_Update(0.0f, 85.0f, 0.0f);
