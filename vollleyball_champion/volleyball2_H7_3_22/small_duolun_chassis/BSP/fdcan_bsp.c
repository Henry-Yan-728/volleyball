#include "fdcan_bsp.h"
#include <string.h> 
#include "vesc_rx.h" // [新增] 引入 VESC 接收头文件，以便调用解析函数
...
static void fdcan_dispatch_router(FDCAN_HandleTypeDef* hfdcan, uint32_t fifo_location)
{
    ...
    // 2. [新增] 处理扩展帧 (专门用于 VESC)
    else if (rx_header.IdType == FDCAN_EXTENDED_ID)
    {
        // VESC 反馈帧 ID 格式通常为: 0x000009xx，其中最后 8 位是 Controller ID
        // 这里简单通过掩码提取 ID。如果你的系统中还有其他扩展帧设备，需要加更严格的判断
        uint8_t motor_id = rx_header.Identifier & 0xFF;
        
        // 调用 vesc_rx.c 中的解析函数
        // 注意：VESC 的 CAN ID 设置里，Motor ID 不要超过 7，否则数组越界
        if (motor_id < 8) { 
