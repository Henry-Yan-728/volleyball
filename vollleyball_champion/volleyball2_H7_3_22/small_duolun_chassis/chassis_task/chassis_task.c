...
void Chassis_Update(float target_vx, float target_vy, float target_vr)
{
    if (Chassis_Should_AutoCenter(target_vx, target_vy, target_vr)) {
        Steering_Wheel_AutoCenter(0, CHASSIS_AUTO_CENTER_ANGLE_DEG);
        Steering_Wheel_AutoCenter(1, CHASSIS_AUTO_CENTER_ANGLE_DEG);
        Steering_Wheel_AutoCenter(2, CHASSIS_AUTO_CENTER_ANGLE_DEG);
        return;
    }
    ...
    Steering_Wheel_Control(1, spd_2, ang_2-60);
    Steering_Wheel_Control(2, spd_3, ang_3+60);
}
