...
//GPIO扫描读取状态与处理函数
#define KEY_DEBOUNCE_TIME  3  // 消抖时间（单位：定时器中断周期，例如10ms则总消抖时间为20ms）
#define KEY_NUM            16
...
Key_State key_state[KEY_NUM] = {0}; // 全局按键状态数组
//GPIO事件处理函数

//mhr个人理解，Key的两个函数都在同时工作，在i<10时处理的时钮子，其余处理按键
void Key_Pressed(int i)
  {
		//如果为两档钮子开关，则对应i=0时不置2即可，即该组钮子开关只有1、3两种状态
		if(i < 10)//钮子开关事件
		{
			if(i%2==0)
			{
...
////		//非压缩传输空间版本，即每个按键占一个传输位，可以同时按下多个按键
		else//按键事件
		{
			TX_buffer[18+i-10]=1;
			BEEP();
		}
			//压缩传输空间版本，即6个按键共用1个传输位，无法同时按下多个按键
//			else//按键事件
//			{
//				TX_buffer[18]=i-10+1;//KEY10-KEY15为按键，按下则代表按键信息传输位置为对应按键序号（1-6），0代表按键都松开
//				BEEP();
//			}
  }
